...
#include "ActsExamples/Vertexing/MyAdaptiveMultiVertexFinder.hpp"
...
MyAdaptiveMultiVertexFinder::MyAdaptiveMultiVertexFinder(
...
MyAdaptiveMultiVertexFinder::makeVertexSeeder() const {
  if (m_cfg.seedFinder == SeedFinder::TruthSeeder) {
...
MyAdaptiveMultiVertexFinder::makeVertexFinder(
...
ProcessCode MyAdaptiveMultiVertexFinder::execute(
  ...
  Acts::Vertex beamConstraint;
  beamspotConstraint.setPosition(beamspotPos);
  beamspotConstraint.setCovariance(beamspotCov);
  
  Options finderOpts(ctx.geoContext, ctx.magFieldContext, beamConstraint, true);
